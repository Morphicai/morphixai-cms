...
## 权限模型第一期：`@optimus/permission` 判定核心（2026-09-06，已归档）

**背景是两个结构性缺失**，不是"想重构"：① 权限码是不透明的扁平字符串
（`OrderManagement`），平台无法回答"这个码属于谁、能做什么、越界没有"——而
一/二/三方都会开发子应用是既定场景；② `perms.includes(code)` 这行判定
**在四个地方各存一份**（api 守卫、partner-service 裸写 introspect、
optimus-ui 的 `filterMenus`、将来的子应用前端），各写各的必然漂移。

**码的形态定为 `namespace:resource:action`。** 三段各解决一件事：namespace 是归属、
resource 是对什么、action 是做什么（当前完全没有动作粒度——按钮权限被移除后，
持有 `OrderManagement` 的人能看能改能删，无法表达"只读"）。

**命名空间是整套设计的安全支点。** 前两版设计里我一直在用"授予即批准"这类机制去补
"三方声明宽松码蹭可见性"的洞；改成"一个服务只能声明自己 namespace 下的码"后，
这个洞**免费消失**，且不需要任何批准流程。`belongsTo()` 就是这条防线。

**四个刻意的取舍**（实现里都有注释）：
1. **中缀通配不支持**（`*:campaign:read` 非法）——它跨越命名空间边界，
   等于开一个绕过归属校验的口子。只允许后缀通配
2. **required 侧不接受通配**——`@RequirePermission("partner:campaign:*")`
   等于声明方自己放宽门槛
3. **`canAny([])` 返回 false**——空数组直觉像"无限制"，实际语义必须是"只有超管可见"。
   这条容易写反
4. **判定永远不抛异常**——拼错的码、缺主体、非法要求一律 fail-closed。
   让守卫因为一个拼错的码抛 500，比返回 403 更糟

**`evaluate` 返回对象而非 boolean**：数据范围（scope）是迟早要加的一维
（"渠道经理只看自己名下的合伙人"），一开始就返回对象，后面加不用改所有调用点。
第一版恒为 `all`——**平台只判"能不能做"，范围由子服务按自己的领域模型翻译成
where 条件**。平台不认识 L2 的字段，这条边界不能破。

**关于 CASL**：结论是平台地基不用它，但理由不是"以前没用过"（那是对现象的观察，
不是对设计的判断）。真正的理由是条件规则的定义方必须懂领域模型，而平台不懂 L2 的
领域模型；且 ability 是对象，跨进程要 packRules 序列化 + 子应用装引擎，
而 `codes: string[]` 通过 introspect 和 embed 握手天然传得过去。
子服务内部要用 CASL 判数据范围是它自己的选择。

**实现期踩到的一个坑**：`Object.assign(fn, { name })` 在编译成 CJS（严格模式）后
...
完整设计（现状梳理、四个视角、明确不做的清单、四期规划）见
`docs/PERMISSION_MODEL.html`。
