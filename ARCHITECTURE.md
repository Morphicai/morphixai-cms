...
**平台判能力，子服务判范围。** 这条分界是分层架构在权限上的直接推论——
条件判定引用的字段属于 L2 的领域模型（`channelManagerId` 只存在于 partner-service 的库里），
平台不该也不能知道。把两者统一就等于让 L1 认识 L2。

- **平台 L1** 判「能不能做这类事」——三段码 `namespace:resource:action` 纯匹配,
  跨进程只传 `string[]`（introspect 与 embed 握手的 `EmbedContext.perms` 已经在传）
- **业务 L2** 判「能看哪些行」——拼进自己的 ORM where，属于领域逻辑

命名空间与服务目录的 `key` 绑定，注册时校验：一个服务只能声明自己命名空间下的码。
这一条免掉了"子应用声明宽松码蹭可见性"所需的整套批准机制。

三类主体（admin / client / service）**统一形态不统一来源**：同一套码语法、同一个判定
函数，但 service 的 grants 不会因为转发了管理员 token 而变多——这是信任模型的红线。

> 完整设计见 **`docs/PERMISSION_MODEL.html`**（含现状、四个视角、数据模型、
> 明确不做的清单、分期）。目标模型已定稿，实现分四期，尚未开始。
