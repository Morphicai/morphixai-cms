...
| 维度 | 状态 |
|---|---|
| 已归档变更 | 17 个（`openspec/changes/archive/`） |
| 规格基线 | 25 个能力（`openspec/specs/`） |
| 活跃变更 | 5 个 |
...
| 权限模型 | 第一期 ✅（`@optimus/permission` 判定核心，36 例单测，零运行时依赖）。**尚未接线**——`@Perm` / `filterMenus` / 数据库都没动，交付物暂无消费方，这是有意的。见 §九 |
...
设计定稿见 **`docs/PERMISSION_MODEL.html`**（现状梳理、四个视角、明确不做的清单）。
这条线不阻塞 ⑧⑨，但**第三期会把菜单四缺陷一并修掉**，越早做越省。

| 期 | 内容 | 状态 |
|---|---|---|
| 一 | `@optimus/permission` 纯核心（`parse`/`matches`/`evaluate`/`definePermissions`/`belongsTo`） | ✅ 2026-09-06 归档 |
| 二 | 40 个扁平码一次性迁到三段式。**纯重命名，不改语义**；三处原子切换：`@Perm` 声明 / `SYSTEM_ROUTES` 的 `code` / `op_sys_role_menu` 数据 | ⏳ |
| 三 | 码字典表 + 菜单表 + 静态菜单入库 + `PermissionModal` 改读字典 | ⏳ |
| 四 | 子应用声明 `menusPath` + 自动发现 + 后台候选列表（菜单注册制） | ⏳ |

**不做兼容层**（2026-09-06 拍板）：项目尚未上线，没有存量用户依赖这批码；
双格式解析的维护成本会一直挂着。代价是第二期必须原子完成——但接口已 fail-closed，
**迁错会立刻 403 而不是静默放行**，错误可见。

### 第三期会顺带修掉的四个已验证缺陷

它们现在都独立存在，不依赖注册制：

1. **顶层菜单泄漏** —— `filterMenus` 里 `children.some(c => c.isDynamicMenu || ...)`
   恒为真，父节点无条件通过第一层过滤，末尾又因为有 `path` 被留下。
   **实测确认**：用户无任何相关码时顶层仍显示且 path 完整。目前只是"菜单显示"泄漏
   （进页仍会 403），但注册制下顶层永远有 `embedUrl`，会从"碰巧没触发"变成"默认发生"
2. **`PermissionModal` 只读 `SYSTEM_ROUTES`** —— 动态服务的码授不出去；
   且勾任意静态项保存会**静默清掉**角色已有的动态码
3. **`orderNum` 是死字段** —— `filterMenus` 完全没有 sort，菜单顺序=数组字面顺序
4. **`config_center` 是替换不是合并** —— `children: dynamicDocMenus` 直接覆盖，
   谁给它加静态子菜单就会被静默冲掉

### 明确不做

CASL 作为平台判定地基。理由**不是**"以前没用过"，而是：条件规则的定义方必须懂领域
模型，而 L1 不认识 L2 的领域模型；且 ability 是对象，跨进程要序列化 + 子应用装引擎，
而 `codes: string[]` 通过 introspect 与 embed 握手天然传得过去。
子服务内部要用 CASL 判自己的数据范围，是它自己的选择。

> 现存那份 CASL 代码是**空转**的（后端 `@CheckPolicies` 零调用点、前端
> `ability.can` 零消费），且 `unified-auth.guard.ts` 里看起来像第三道防线。
> 清理它和选型是两件事，应单独做一次——混进重构就说不清 403 是删错了还是新模型的问题。
