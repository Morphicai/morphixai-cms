...
- [x] 4.1 role 1 补权限码 `TranslationManagement`（INSERT，记录回滚语句）
- [x] 4.2 前端新增「翻译管理」菜单与页面（权限码控制显隐与直达），iframe 地址可配置默认 5181，目标未启动时显示提示与启动指引
- [x] 4.3 验收：从 CMS 菜单进入完成一次真实批量翻译；无权限角色菜单不可见且直达被拒

## 5. 收尾

- [x] 5.1 axios transformResponse 修复：非 JSON 上游错误原样透传，不再伪装成解析失败
- [x] 5.2 已初始化系统隐藏「系统安装」菜单项
- [x] 5.3 全流程验收：登录 → 越权 403 → AI 生成入库 → 翻译工作台走通；以观测工具留存验收会话
- [x] 5.4 本迭代改动分模块提交（中性命名），更新 PROJECT_STATUS
