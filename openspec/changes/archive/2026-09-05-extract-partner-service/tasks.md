...
- [x] 4.5 partner-service 自己实现管理页(合伙人列表/团队/冻结解冻/渠道、积分调试页、外部任务审核列表),复用既有 `EmbedFrame`/`@optimus/admin-embed` 握手协议,不改协议本身——留到第 5 组服务目录接入时一起做,管理页需要先有 embed 入口才有意义单独验证。**实际在第 5 组随 `admin-app/`(独立 Vite + React 18 + antd 5)一起交付**,构建产物落 `public/admin`
