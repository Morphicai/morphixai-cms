...
- [ ] 5.2 `optimus-ui` 里原 partner-admin/points/external-task-admin 相关的静态路由节点下线,菜单改走动态 embed 入口(参照 demo-activity 迁移时 routes.js 的改法)。**执行时发现规模问题,暂停等待决策,未删除任何静态路由**:三个静态路由节点(`partner_management`→`/biz/partner`、`external_task_review`→`/biz/external-task-review`、`partner_data_management`→`/sys/partner-data`)背后是 `pages/partner/`(Dashboard/PartnerList/PartnerDetail/ChannelList/PointsHistory/TaskLogs/TeamMembers,约 2300 行)+ `pages/external-task-review/`(含筛选/统计/审核弹窗,约 500 行)这套完整 React 管理界面;而 4.5/5.4 阶段做的 embed 页只是一个 158 行的纯 vanilla JS 验证性 stub(只有合伙人列表+冻结解冻、外部任务列表+通过驳回两张裸表格,没有团队仪表板、合伙人详情、渠道管理、积分明细、任务日志、成员管理,也没有筛选/统计/审核备注弹窗)。现在下线静态路由会造成真实功能回归,和第 8 组验收标准 8.1("经 embed 页操作与迁移前行为一致,不回归")直接冲突。需要先决定:(a) 把 embed 页扩到功能对等再删旧路由,(b) 暂缓 5.2,先并行保留新旧两个入口直到 8.1 验收时再统一处理,(c) 重新评估 embed 页要不要从"零构建 vanilla JS"换成小型 React 打包(牵动这套 embed 模式的既定约定,影响面更大)。三个选项工作量和影响范围差异很大,已停下来等用户拍板,没有擅自选一个默认方案往下做
