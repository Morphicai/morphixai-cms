...
1. **导出数据库**: 调 `mysqldump` **npm 包**（`import mysqldump from "mysqldump"`）导出为 SQL 文件——不是 shell 调同名 CLI，运行环境里不需要装 mysql-client
...
- **权限**: 类级 `@Perm("DatabaseBackup")`，需要 `DatabaseBackup` 权限码
- **说明**: 立即执行一次数据库备份
...
- **权限**: 类级 `@Perm("DatabaseBackup")`，需要 `DatabaseBackup` 权限码
- **说明**: 返回加密文件（`.enc`）的直接下载链接，不解密。要拿明文用上面的 `download` 接口
...
4. **权限控制**: 
   - 控制器类级挂了 `@Perm("DatabaseBackup")`，只有带该权限码的角色能访问；
     不要在方法上加 `@AllowNoPerm`，方法级放行会盖掉类级的权限门
   - 定期审查备份访问日志
...
- 定时任务: `backup-scheduler.service.ts`
