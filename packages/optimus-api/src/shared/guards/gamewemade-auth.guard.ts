...
import {
    REQUIRE_GAMEWEMADE_AUTH_KEY,
    GameWemadeAuthOptions,
 ...
 *     // 签名已验证，可以从 req.gameWemadeUser 获取用户信息
 *     const user = req.gameWemadeUser!;
 *   }
 * }
 * ```
 */
@Injectable()
export class GameWemadeAuthGuard implements CanActivate {
    private readonly logger = new Logger(GameWemadeAuthGuard.name);
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        // 获取装饰器配置
        const options = this.reflector.getAllAndOverride<GameWemadeAuthOptions>(REQUIRE_GAMEWEMADE_AUTH_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        if (nodeEnv === "development") {
            ...
            const uidField = options.uidField || GAMEWEMADE_UID_HEADER;
            const uid = request.headers?.[uidField.toLowerCase()] as string | undefined;

            if (uid) {
                // 将用户信息附加到请求对象上
                request.gameWemadeUser = { uid };
        ...
        if (!openKey) {
            this.logger.error("GameWemade SDK OpenKey 未配置");
        ...
        // 生成签名
        try {
            const calculatedSign = generateSign(signParams, openKey);
...
/**
 * 扩展 Request 类型，添加 gameWemadeUser 属性
 */
declare global {
