 ...
 *
 * 本想在 mysql2 升 3.x 后用 idleTimeout 替掉本服务——实测被打脸：
 * 3.23.4 配 maxIdle<connectionLimit 的回收器在满池并发后回收不完整，
 * 且回收过的池再来一波并发会整体挂死（checkout 永久排队，查询根本
 * 到不了 server）。复现步骤：10 并发打满池 → 闲置 80s → 再 10 并发全超时。
 * 所以回收器不开（不配 maxIdle），池的活性由本服务兜底——它的整池重建
 * 恰好也能救回上述挂死状态。
 */
@Injectable()
