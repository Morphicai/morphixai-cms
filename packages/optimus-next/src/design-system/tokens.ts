...
export function generateCSSVariables(tokens: DesignTokens): Record<string, string> {
  ...
  Object.entries(tokens.colors).forEach(([colorName, colorShades]) => {
    if (colorName === 'semantic') {
      Object.entries(colorShades).forEach(([semanticName, semanticShades]) => {
        Object.entries(semanticShades as Record<string, string>).forEach(([shade, value]) => {
          cssVars[`--color-${semanticName}-${shade}`] = value;
        });
      });
    } else {
  ...
  // 断点变量
  Object.entries(tokens.breakpoints).forEach(([breakpoint, value]) => {
    cssVars[`--breakpoint-${breakpoint}`] = value;
  });
