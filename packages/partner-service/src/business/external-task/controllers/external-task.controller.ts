...
import { uploadFileViaOptimusApi, extractClientToken } from "../../../shared/utils/optimus-api-client";
...
export class ExternalTaskController {
    ...
    async uploadProof(@Req() req: any, @UploadedFile() file: Express.Multer.File): Promise<ResultData> {
        ...
        try {
            ...
            const uploaded = await uploadFileViaOptimusApi(file, token);

            this.logger.log(`External task proof uploaded: ${uploaded.url}`);
