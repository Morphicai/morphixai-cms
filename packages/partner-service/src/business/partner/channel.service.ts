...
import { shortenViaOptimusApi } from "../../shared/utils/optimus-api-client";
...
export class ChannelService {
    ...
    async createChannel(
        partnerId: string,
        name: string,
        customChannelCode: string | undefined,
        clientToken: string,
    ): Promise<PartnerChannelEntity> {
        // 验证合伙人存在且状态为active
        ...
        // 生成推广链接（短链token）
        const shortUrl = await this.generateReferralLink(partner.partnerCode, channelCode, clientToken);
     ...
     * 生成推广链接（使用短链服务）
     */
    async generateReferralLink(partnerCode: string, channelCode: string, clientToken: string): Promise<string> {
        try {
            // 短链不随迁移复制(见 design.md 3.5 节),改成 HTTP 调 optimus-api
            const params = {
                inviterCode: partnerCode,
                channelCode: channelCode,
            };

            const result = await shortenViaOptimusApi(
                params,
