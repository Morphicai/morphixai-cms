...
import { ClientUserAuth } from "../../shared/decorators/auth.decorators";
import { ResultData } from "../../shared/utils/result";
import { ApiResult } from "../../shared/decorators/api-result.decorator";
import { extractClientToken } from "../../shared/utils/optimus-api-client";

// 系统级 @OperationLog/OperationLogInterceptor(写 op_sys_operation_log)是
// optimus-api 侧的横切能力,没有随迁移带过来——见 design.md 3.5 节的判断:
// 审计日志和存储/短链一样是平台级能力,不该给每个业务子服务各开一份实现。
// 如果后续要在 partner-service 里恢复操作审计,应该设计成调用平台侧的
// 日志接口,而不是复制一份 OperationLogInterceptor
@ApiTags("合伙人计划")
@ApiBearerAuth()
...
- 如果已有上级，则忽略本次的 inviterCode`,
    ...
    async createChannel(@Body() dto: CreateChannelDto, @Req() req: any): Promise<ResultData> {
        ...
        if (!clientToken) {
            return ResultData.fail(401, "登录状态已失效，请重新登录");
        }
        const channel = await this.channelService.createChannel(profile.partnerId, dto.name, dto.channelCode, clientToken);
