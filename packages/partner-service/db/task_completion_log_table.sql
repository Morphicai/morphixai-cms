...
--
-- 本文件与 packages/optimus-api/db/task_completion_log_table.sql 是同一份迁移(dev 阶段
-- partner-service 和 optimus-api 共用同一个 MySQL 实例,只需要在其中一边执行一次即可)。
-- 这里放一份是为了这张表的 schema 归属清楚地跟着实体定义走(TaskCompletionLogEntity 已经
-- 搬到 partner-service),不是要在同一个库里重复建表。
