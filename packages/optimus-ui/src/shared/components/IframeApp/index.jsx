 ...
 * EmbedFrame 是参数化组件:url 可来自服务目录(动态入口 /embed/:key)或
 * 代码登记(createIframeApp 工厂,静态接入的兼容形态)。
 *
 ...
 */
export const EmbedFrame = ({ url, title = '子应用' }) => {
...
/** 静态接入的兼容形态:routes.js 里 `Xxx: createIframeApp({ url })` 登记 */
const createIframeApp = ({ url, title = '子应用' }) => {
  const IframeApp = () => <EmbedFrame url={url} title={title} />;
  return IframeApp;
};
